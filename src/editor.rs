...
pub struct Editor {
    ...
    pub cursor: Cursor,
    pub is_dirty: bool,
    // Thêm trường lưu trữ lệnh biên dịch tùy biến cho riêng file hiện tại
    pub custom_build_cmd: Option<String>,
    // Viewport scroll offsets to fix the BlindCursor bug
...
impl Editor {
    ...
    /// Keep the cursor within viewable window bounds
    pub fn scroll_into_view(&mut self, visible_width: usize, visible_height: usize) {
        ...
        } else if self.cursor.col >= self.col_offset + visible_width {
            self.col_offset = self.cursor.col - visible_width + 1;
        }
    }
    ...
    /// Delete character behind cursor (Backspace)
    ...
    /// Safely move cursor left, right, up, down without overflow panic
